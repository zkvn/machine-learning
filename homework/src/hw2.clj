...
(def Y  (parser "data/newsgrouplabels.txt" ))
(def X  (parser "data/vocabulary.txt" ) )
(def trainLabel  (parser "data/train.label" ))
(def testLabel  (parser "data/test.label" ))

(defn dataFn [dataFile]
  (let  [  data (parser  dataFile )]
    (reduce #( assoc-in %1 [ (nth %2 0) (nth %2 1)]   (nth %2 2))  {} (vals data)  )))

(def testData (dataFn  "data/test.data" ))
(def trainData (let [  data (parser "data/train.data" )
                     ]
